# Heroes part 2 - [`react-router`](https://reactrouter.com/en/main/getting-started/overview)

We are back in the `Heroes` component, and this time we have routing capabilities. On initial render, `Heroes` displays its child `HeroList` component. We need it to be able to display `HeroDetail` when clicking the + button of the `ListHeader`. Then we need `HeroList` displayed again when clicking the refresh button of the `ListHeader`. `Cancel` buttons should go back from `HeroDetail` to `HeroList`.  Create a new branch `feat/Heroes-part2`
...
```tsx
const handleSelectHero = (id: string) => navigate(`/heroes/edit-hero/${id}`)
```
...
In order to use the path attribute, destructure the `id` out of `useParams()` with `const { id } = useParams()`, because Instead of relying on the hero data, we want to rely on the path attribute that we get from the url. This also has a side benefit of being able to directly navigate to a url (Green 8).
